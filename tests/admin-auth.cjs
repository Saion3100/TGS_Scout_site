...
(async () => {
    ...
    try {
        ...
        assert.doesNotMatch(profile.text, /name="delete_student"/);
        forbidden(await student('/admin.php', { delete_student: '1', original_id: '1', csrf_token: csrf }));
        forbidden(await student('/teams_admin.php', { delete_team: '1', id: 't01', csrf_token: csrf }));
        ...
        forbidden(await admin('/admin.php', { delete_student: '1', original_id: '1', csrf_token: 'invalid' }));
        assert.equal(read('students').length, 2);
        assert.match((await admin('/admin.php', { delete_student: '1', original_id: '1', csrf_token: adminCsrf })).location, /deleted=1/);
        assert.equal(read('students').length, 1);
        assert.equal(read('students')[0].id, '2');
        assert.equal(read('featured_students').length, 0);
        assert.equal(read('mapping').length, 0);
        assert.equal(read('qr_codes').find(q => q.id === 'student-1').is_active, false);
        assert.match((await student('/teams_admin.php')).location, /admin\.php/);
        write('mapping', [{ student_id: '2', team_id: 't01', role: 'test' }, { student_id: '2', team_id: 't02', role: 'test' }]);
        forbidden(await admin('/teams_admin.php', { delete_team: '1', id: 't02', csrf_token: 'invalid' }));
        assert.match((await admin('/teams_admin.php', { delete_team: '1', id: 't02', csrf_token: adminCsrf })).location, /deleted=1/);
        assert.equal(read('teams').length, 1);
        assert.equal(read('mapping').length, 1);
        assert.equal(read('mapping')[0].team_id, 't01');
        assert.equal(read('qr_codes').find(q => q.id === 'team-t02').is_active, false);
        const newTeam = await admin('/teams_admin.php?new=1', { ...workFields, mode: 'new', csrf_token: adminCsrf });
        assert.match(newTeam.location, /id=t03/);
        assert.match((await admin('/teams_admin.php', { delete_team: '1', id: 'missing', csrf_token: adminCsrf })).location, /status=404/);
