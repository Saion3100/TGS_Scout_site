...
{
    ?><script src="<?= e(assetUrl('vendor/qrcode.min.js')) ?>"></script><script src="<?= e(assetUrl('qr-admin.js')) ?>"></script><script src="<?= e(assetUrl('student-photo-admin.js')) ?>"></script><script src="<?= e(assetUrl('admin-list-sort.js')) ?>"></script><script src="<?= e(assetUrl('searchable-select.js')) ?>"></script></main></body></html><?php
}
...
<section class="admin-shell">
  ...
  <div class="admin-grid">
    <aside class="admin-list" data-admin-sort-list="students"><div class="admin-list-head"><strong>登録学生</strong><span><?= count($students) ?>名</span></div><label class="admin-list-sort">並び順<select data-admin-sort><option value="id">ID昇順</option><option value="name">名前昇順（ふりがな）</option></select></label><label class="admin-list-search">検索<input type="search" data-admin-search placeholder="ID・名前・ふりがな"></label><p data-admin-search-empty role="status" hidden>該当する項目がありません。</p><a class="button button-primary" href="<?= e(url('admin.php')) ?>?new=1">＋ 新しい学生を追加</a><?php foreach ($students as $item): ?><a data-sort-id="<?= e($item['id'] ?? '') ?>" data-sort-name="<?= e(preg_replace('/\s+/u', '', trim((string) ($item['name_kana'] ?? ''))) ?: ($item['name'] ?? '')) ?>" class="<?= !$isNew && $selectedId === ($item['id'] ?? '') ? 'is-current' : '' ?>" href="<?= e(url('admin.php')) ?>?id=<?= e($item['id'] ?? '') ?>"><span><strong><?= e($item['id'] ?? '') ?>　<?= e($item['name'] ?? '(氏名なし)') ?></strong><small><?= e(listText($item['role'] ?? [])) ?></small></span><span class="admin-status <?= !empty($item['is_active']) ? 'is-public' : '' ?>"><?= !empty($item['is_active']) ? '公開' : '非公開' ?></span></a><?php endforeach; ?></aside>
