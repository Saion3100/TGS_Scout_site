...
<fieldset><legend><span>01</span>基本情報</legend><div class="admin-form-grid"><label>作品名 <b>*</b><input name="game_name" value="<?=e($team['game_name'])?>" required></label><label>チーム名 <b>*</b><input name="team_name" value="<?=e($team['team_name'])?>" required></label><label>ジャンル <b>*</b><textarea name="genre" rows="3" placeholder="1行に1つ入力" required><?=e(teamAdminListText($team['genre']))?></textarea></label><label>プレイ人数 <b>*</b><input name="players" value="<?=e($team['players'])?>" required></label><label>対応機種<textarea name="platforms" rows="3"><?=e(teamAdminListText($team['platforms']))?></textarea></label><label>使用エンジン <b>*</b><textarea name="engine" rows="3" placeholder="1行に1つ入力" required><?=e(teamAdminListText($team['engine']))?></textarea></label><label>開発開始日 <b>*</b><input placeholder="2026-04-01 / 2026-04 / 2026" name="development_start_date" value="<?=e($team['development_start_date'] ?? '')?>" required></label><label>開発終了日（空欄の場合は開発中）<input placeholder="2026-04-01 / 2026-04 / 2026" name="development_end_date" value="<?=e($team['development_end_date'] ?? '')?>"></label><label>試遊台番号（任意・重複可・公開ページでは非表示）<input name="booth_no" value="<?=e($team['booth_no'] ?? '')?>"></label><label>テーマ<select name="theme"><?php foreach(['red','navy','yellow'] as $theme):?><option value="<?=e($theme)?>" <?=($team['theme']??'')===$theme?'selected':''?>><?=e($theme)?></option><?php endforeach;?></select></label></div></fieldset>
