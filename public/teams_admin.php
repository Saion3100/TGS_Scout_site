...
$repo=repository('teams'); $teams=$repo->all(); $error=''; $notice=''; $selectedId=(string)($_GET['id']??''); $isNew=isset($_GET['new']); $team=$isNew?[]:($selectedId!==''?findById($teams,$selectedId):null);
if (!isAdminUser()) {
    if ($isNew || ($_POST['mode'] ?? '') === 'new' || isset($_POST['save_members'])) redirectToError(403);
    if ($selectedId !== '' && !canEditTeam($selectedId)) redirectToError(403);
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && (!is_string($_POST['id'] ?? null) || !canEditTeam($_POST['id']))) redirectToError(403);
    $teams = array_values(array_filter($teams, static fn(array $item): bool => canEditTeam((string) $item['id'])));
}
$students = isAdminUser() ? data('students') : [];
usort($students, static fn(array $a, array $b): int => strnatcmp((string) $a['id'], (string) $b['id']));
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['save_team'])) {
    ...
    try {
        ...
        $endDate = teamAdminText('development_end_date');
        foreach (['開発開始日' => $values['development_start_date'], '開発終了日' => $endDate] as $label => $dateValue) {
            if ($dateValue === '') continue;
            $date = DateTimeImmutable::createFromFormat(strlen($dateValue) === 4 ? '!Y' : (strlen($dateValue) === 7 ? '!Y-m' : '!Y-m-d'), $dateValue);
            if (!$date || $date->format(strlen($dateValue) === 4 ? 'Y' : (strlen($dateValue) === 7 ? 'Y-m' : 'Y-m-d')) !== $dateValue) throw new InvalidArgumentException($label.'を正しい日付で入力してください。');
        }
        if ($endDate !== '' && $endDate < $values['development_start_date']) throw new InvalidArgumentException('開発終了日は開発開始日以降にしてください。');
        ...
        $team=['id'=>$id,'team_name'=>$values['team_name'],'game_name'=>$values['game_name'],'genre'=>teamAdminList('genre'),'players'=>$values['players'],'platforms'=>teamAdminList('platforms'),'engine'=>teamAdminList('engine'),'development_start_date'=>$values['development_start_date'],'development_end_date'=>$endDate,'booth_no'=>teamAdminText('booth_no'),'catchcopy'=>$values['catchcopy'],'description'=>$values['description'],'highlights'=>teamAdminList('highlights'),'controls'=>teamAdminList('controls'),'screenshots'=>$existing['screenshots']??[],'theme'=>teamAdminText('theme')?:'red','video_url'=>$video];
        $team = array_replace($existing, $team);
        if (!isAdminUser()) {
            if (!canEditTeam($id)) redirectToError(403);
            $team['booth_no'] = $existing['booth_no'] ?? '';
        }
        unset($team['production_period']);
        $repo->saveById($team,$isNew);
        ...
        $error = $exception->getMessage();
        $team=['id'=>$isNew?'':teamAdminText('id'),'game_name'=>teamAdminText('game_name'),'team_name'=>teamAdminText('team_name'),'genre'=>teamAdminList('genre'),'players'=>teamAdminText('players'),'platforms'=>teamAdminList('platforms'),'engine'=>teamAdminList('engine'),'development_start_date'=>teamAdminText('development_start_date'),'development_end_date'=>teamAdminText('development_end_date'),'booth_no'=>teamAdminText('booth_no'),'theme'=>teamAdminText('theme')?:'red','catchcopy'=>teamAdminText('catchcopy'),'description'=>teamAdminText('description'),'highlights'=>teamAdminList('highlights'),'controls'=>teamAdminList('controls'),'video_url'=>teamAdminText('video_url')];
...
<?php renderManagementTabs('teams'); ?>
<?php if($notice):?><p class="admin-success"><?=e($notice)?></p><?php endif;?><?php if($error):?><p class="admin-error" role="alert"><?=e($error)?></p><?php endif;?>
<div class="admin-grid"><aside class="admin-list" data-admin-sort-list="teams"><div class="admin-list-head"><strong>登録作品</strong><span><?=count($teams)?>件</span></div><label class="admin-list-sort">並び順<select data-admin-sort><option value="id">ID昇順</option><option value="name">作品名昇順</option></select></label><label class="admin-list-search">検索<input type="search" data-admin-search placeholder="ID・作品名・チーム名"></label><p data-admin-search-empty role="status" hidden>該当する項目がありません。</p><?php if (isAdminUser()): ?><a class="button button-primary <?= $isNew?'is-current':'' ?>" href="<?=e(url('teams_admin.php'))?>?new=1">＋ 新しい作品を追加</a><?php endif; ?><?php foreach($teams as $item):?><a data-sort-id="<?= e($item['id'] ?? '') ?>" data-sort-name="<?= e($item['game_name'] ?? '') ?>" class="<?= $selectedId===($item['id']??'')?'is-current':''?>" href="<?=e(url('teams_admin.php'))?>?id=<?=e($item['id'])?>"><span><strong><?=e($item['id'] ?? '')?>　<?=e($item['game_name'])?></strong><small><?=e($item['team_name'])?></small></span><span class="admin-status is-public">公開</span></a><?php endforeach;?></aside>
<div class="admin-editor"><?php if($team!==null):$team+=['id'=>'','game_name'=>'','team_name'=>'','genre'=>[],'players'=>'','platforms'=>[],'engine'=>[],'development_start_date'=>'','development_end_date'=>'','booth_no'=>'','theme'=>'red','catchcopy'=>'','description'=>'','highlights'=>[],'controls'=>[],'video_url'=>'']; $teamQrUrl=managedQrUrl('team-'.$team['id']);?><div class="admin-editor-head"><div><p class="section-number"><?= $isNew?'NEW WORK':'EDIT WORK' ?></p><h2><?= $isNew?'作品を新規追加':e($team['game_name']) ?></h2></div><span>必須項目 <b>*</b></span></div><form method="post" class="admin-student-form"><input type="hidden" name="csrf_token" value="<?=e($_SESSION['csrf_token'])?>"><input type="hidden" name="mode" value="<?= $isNew?'new':'edit' ?>"><input type="hidden" name="id" value="<?=e($team['id'])?>">
<fieldset><legend><span>01</span>基本情報</legend><div class="admin-form-grid"><label><span>作品名 <b>*</b></span><input name="game_name" value="<?=e($team['game_name'])?>" required></label><label><span>チーム名 <b>*</b></span><input name="team_name" value="<?=e($team['team_name'])?>" required></label><label><span>ジャンル <b>*</b></span><textarea name="genre" rows="3" placeholder="1行に1つ入力" required><?=e(teamAdminListText($team['genre']))?></textarea></label><label><span>プレイ人数 <b>*</b></span><input name="players" value="<?=e($team['players'])?>" required></label><label><span>対応機種</span><textarea name="platforms" rows="3"><?=e(teamAdminListText($team['platforms']))?></textarea></label><label><span>使用エンジン <b>*</b></span><textarea name="engine" rows="3" placeholder="1行に1つ入力" required><?=e(teamAdminListText($team['engine']))?></textarea></label><label><span>開発開始日 <b>*</b></span><input placeholder="2026-04-01 / 2026-04 / 2026" name="development_start_date" value="<?=e($team['development_start_date'] ?? '')?>" required></label><label><span>開発終了日（空欄の場合は開発中）</span><input placeholder="2026-04-01 / 2026-04 / 2026" name="development_end_date" value="<?=e($team['development_end_date'] ?? '')?>"></label><?php if (isAdminUser()): ?><label><span>試遊台番号（任意・重複可・公開ページでは非表示）</span><input name="booth_no" value="<?=e($team['booth_no'] ?? '')?>"></label><?php endif; ?><label><span>テーマ</span><select name="theme"><?php foreach(['red','navy','yellow'] as $theme):?><option value="<?=e($theme)?>" <?=($team['theme']??'')===$theme?'selected':''?>><?=e($theme)?></option><?php endforeach;?></select></label></div></fieldset>
<fieldset><legend><span>02</span>紹介内容</legend><div class="admin-form-grid"><label class="admin-span-2"><span>キャッチコピー <b>*</b></span><input name="catchcopy" value="<?=e($team['catchcopy'])?>" required></label><label class="admin-span-2"><span>作品説明 <b>*</b></span><textarea name="description" rows="5" required><?=e($team['description'])?></textarea></label><label><span>見どころ</span><textarea name="highlights" rows="5"><?=e(teamAdminListText($team['highlights']))?></textarea></label><label><span>操作方法</span><textarea name="controls" rows="5"><?=e(teamAdminListText($team['controls']))?></textarea></label><label class="admin-span-2"><span>動画URL</span><input type="url" name="video_url" value="<?=e($team['video_url'])?>"></label></div></fieldset>
<?php if(!$isNew): ?>
<?php if (isAdminUser()): ?><fieldset id="project-members"><legend><span>03</span>プロジェクトメンバー</legend>
...
<label>メンバー <?= $slot + 1 ?><select data-searchable-select data-member-student aria-describedby="member-warning-<?= $slot ?>" form="project-members-form" name="members[<?= $slot ?>][student_id]">
<option value="">未選択</option>
<?php foreach ($students as $student): ?><option data-search="<?= e($student['name_kana'] ?? '') ?>" data-public="<?= isStudentPublic($student) ? '1' : '0' ?>" value="<?= e($student['id']) ?>" <?= (string) $student['id'] === (string) $member['student_id'] ? 'selected' : '' ?>><?= e($student['id']) ?>：<?= e($student['name']) ?><?= !isStudentPublic($student) ? '［非公開］' : '' ?></option><?php endforeach; ?>
</select></label>
