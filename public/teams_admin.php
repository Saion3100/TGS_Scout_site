...
<nav class="admin-tabs" aria-label="管理データ"><a href="<?= e(url('admin.php')) ?>">学生</a><a href="<?= e(url('admin.php')) ?>?section=featured">注目学生</a><a class="is-active" aria-current="page" href="<?= e(url('teams_admin.php')) ?>">作品</a><a href="<?= e(url('exhibited_admin.php')) ?>">出展作品</a></nav>
<?php if($notice):?><p class="admin-success"><?=e($notice)?></p><?php endif;?><?php if($error):?><p class="admin-error" role="alert"><?=e($error)?></p><?php endif;?>
<div class="admin-grid"><aside class="admin-list" data-admin-sort-list="teams"><div class="admin-list-head"><strong>登録作品</strong><span><?=count($teams)?>件</span></div><label class="admin-list-sort">並び順<select data-admin-sort><option value="id">ID昇順</option><option value="name">作品名昇順</option></select></label><label class="admin-list-search">検索<input type="search" data-admin-search placeholder="ID・作品名・チーム名"></label><p data-admin-search-empty role="status" hidden>該当する項目がありません。</p><a class="button button-primary <?= $isNew?'is-current':'' ?>" href="<?=e(url('teams_admin.php'))?>?new=1">＋ 新しい作品を追加</a><?php foreach($teams as $item):?><a data-sort-id="<?= e($item['id'] ?? '') ?>" data-sort-name="<?= e($item['game_name'] ?? '') ?>" class="<?= $selectedId===($item['id']??'')?'is-current':''?>" href="<?=e(url('teams_admin.php'))?>?id=<?=e($item['id'])?>"><span><strong><?=e($item['id'] ?? '')?>　<?=e($item['game_name'])?></strong><small><?=e($item['team_name'])?></small></span><span class="admin-status is-public">公開</span></a><?php endforeach;?></aside>
