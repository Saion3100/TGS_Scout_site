...
<?php foreach ($roles as $i => $role): $roleStudents = array_values(array_filter($students, fn(array $s): bool => $s['role'] === $role)); ?>
<section class="role-section section-pad <?= $i % 2 ? 'soft-section' : '' ?>">
    <div class="section-head"><div><p class="section-number">0<?= $i + 1 ?> — CREATOR ROLE</p><h2><?= e($role) ?></h2></div><span class="count-label"><?= count($roleStudents) ?> CREATORS</span></div>
